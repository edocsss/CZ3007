...
public class ExprCodeGenerator extends Visitor<Value> {
	...
	public Value visitIntLiteral(IntLiteral nd) {
		...
		Value intLiteral = wrap(IntConstant.v(nd.getValue()));
		return intLiteral;
	}
	...
	public Value visitStringLiteral(StringLiteral nd) {
		/* TODO: return something meaningful here */
		Value stringLiteral = wrap(StringConstant.v(nd.getValue()));
	...
	public Value visitBooleanLiteral(BooleanLiteral nd) {
		...
		if (b) {
			return wrap(IntConstant.v(1));
		} else {
			return wrap(IntConstant.v(0));
		}
	}
	...
	public Value visitArrayIndex(ArrayIndex nd) {
		...
		Value baseValue = wrap(nd.getBase().accept(this));
		Value accessor = wrap(nd.getIndex().accept(this));
		return Jimple.v().newArrayRef(baseValue, accessor);
	}
	...
	public Value visitNegExpr(NegExpr nd) {
		...
		Value val = wrap(nd.getOperand().accept(this));
